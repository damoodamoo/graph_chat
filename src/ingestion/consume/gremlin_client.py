...
class GremlinService:
    ...
    def execute_query(self, query: str) -> list:
        """
        Execute a Gremlin query with retry for rate limiting and connection errors.
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if "403" in error_str or "Forbidden" in error_str:
                    ...
                    self._reset_client()
                    if attempt < self.MAX_RETRIES - 1:
                        print(f"Connection error (403), reconnecting (attempt {attempt + 1}/{self.MAX_RETRIES})")
        ...
        raise RuntimeError("Unexpected state: no result and no exception after retries")
    ...
    def _upsert_node(self, label: str, node_type: str, data: dict) -> None:
        ...
        query = (
            f"g.V('{self._escape(label)}')"
            f".fold()"
            f".coalesce("
            f"  unfold(){properties},"
            f"  addV('{self._escape(node_type)}').property('id', '{self._escape(label)}').property('partitionKey', '{self._escape(node_type)}'){properties}"
            f")"
        )
    ...
    def _delete_node(self, label: str) -> None:
        ...
        query = f"g.V('{self._escape(label)}').drop()"
        self.execute_query(query)
